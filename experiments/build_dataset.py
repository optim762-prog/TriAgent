...
The pre-built dataset is already included in datasets/ — re-generation is not
required for reproducing the paper results.
...
Phases:
  ...
  C  Paraphrase generation via LLM (default: qwen-local via llama.cpp)
  D  Ground truth file: derived from seed tables, no LLM involved

Usage:
    # Local Qwen3-27B (llama.cpp running on localhost:8080):
    python experiments/build_dataset.py

    # Inside Docker (llama.cpp on triagent-llama-cpp-server):
    python experiments/build_dataset.py --llamacpp-url http://triagent-llama-cpp-server:8080/v1

    # DeepSeek cloud:
    python experiments/build_dataset.py --provider deepseek --backbone deepseek-chat
...
if __name__ == "__main__":
    ...
    parser.add_argument("--provider", default="llamacpp_b",
                        choices=["llamacpp_b", "deepseek"])
    parser.add_argument("--backbone", default="qwen-local",
                        help="Model alias / name (e.g. qwen-local, deepseek-chat)")
